...
module.exports = {
    ...
    '9': emoji.dokuz}[d];
        ...
        if (kurulus > 604800000) {
            member.roles.add(config.register.kayitsiz);
            member.roles.add(config.register.kayitsiz);
            member.setNickname(config.register.JoinName);
            member.guild.channels.cache.get(config.kanallar.KayitKanali).send(`
    
${emoji.crown} ${member} **${member.guild.name}** Sunucumuza Hoş Geldin, ${emoji.hosgldinn}
Seninle beraber sunucumuz ${üyesayısı} üye sayısına ulaştı ${emoji.tada}
    
${emoji.star} Hesabın __${kuruluş}__ tarihinde \`(${zaman})\` oluşturulmuş. **(Güvenli** ${emoji.nbdy_onay}**)**
${emoji.nbdy_unlem} Kayıt işleminden sonra <#${config.kanallar.rules}> kanalına göz atmayı unutmayın.
    
${emoji.kirmiziok} Tagımıza ulaşmak için herhangi bir kanala \`.tag\` yazabilirsiniz. <@&${config.staffroles.KayitYetkiliRole}>
\`\`\`fix
Kayıt olduktan sonra kuralları okuduğunuzu kabul edeceğiz ve içeride yapılacak cezalandırma işlemlerini bunu göz önünde bulundurarak yapacağız. ${member.guild.name}\`\`\`
    `);
            ...
            member.setNickname(config.register.ŞüpheliName);
            member.roles.add(config.register.şüpheliRolü);
            member.setNickname(config.register.JoinName);
            member.guild.channels.cache.get(config.kanallar.KayitKanali).send(`${emoji.nbdy_unlem} ${member}, kullanıcısı sunucuya katıldı hesabı **${zaman2}** önce açıldığı için şüpheli.`)
        }
    }
    }